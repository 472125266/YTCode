//
//  Combination_ProvideViewController.m
//  房贷计算器APP－YT
//
...
#import "Combination_ProvideViewController.h"

@interface Combination_ProvideViewController ()

@end

@implementation Combination_ProvideViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    //初始化数据
    [self Combination_InitViewNeedThing];
}
#pragma mark -
#pragma mark 初始化数据
-(void)Combination_InitViewNeedThing{
    self.navigationItem.title = @"组合贷款";
    _Section_1DataSource = [[NSMutableArray alloc]initWithObjects:Combination_Section_1_DataSourceArray, nil];
    _Section_2DataSource = [[NSMutableArray alloc]initWithObjects:Combination_Section_2_DataSourceArray, nil];
    ...
    //TableView的代理设置
    _Combination_TableView.delegate = self;
    _Combination_TableView.dataSource = self;
    //使其利率/折扣为默认数据
    _CPF_LoanInterest = CPF_AccumulationFund_Default;
    _Trade_LoanInterest = Trade_AccumulationFund_Default;
    _Trade_InterestRateDiscount = Trade_AccumulationFund_Discount;
}
#pragma mark -
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *strCell = @"Combination_Cell";
    ...
    if (cell == nil) {
        ...
        if(indexPath.section == 1) {
            ...
            if(indexPath.row == 0){
                strTemp = [NSString stringWithFormat:@"%.2f",(_Trade_LoanInterest * _Trade_InterestRateDiscount)];
            }
        ...
        if(indexPath.section == 2) {
            ...
            if(indexPath.row == 0){
                [customCell1 updatashowlabelInterest:_showSection_3_Row_1 Money:[NSString stringWithFormat:@"%.2f",_CPF_EveryMonthMoney+_Trade_EveryMonthMoney]];
            }
            else if(indexPath.row == 1){
...
#pragma mark 分区一数据订制带TextField 数据分析
-(UITableViewCell *)CustomSection_1_HaveTextField:(NSInteger)indexPathRow Cell:(UITableViewCell *)cell{
    ...
    NSString *tempStr2;
    if (indexPathRow == 0) {
        tempStr2 = [NSString stringWithFormat:@"%.2f",_CPF_LoanMoney];
        ...
        tempStr2 = [NSString stringWithFormat:@"%.2f",_CPF_LoanInterest];
        ...
        tempStr2 = [NSString stringWithFormat:@"%.2f",_Trade_LoanMoney];
        ...
        tempStr2 = [NSString stringWithFormat:@"%.2f",_Trade_LoanInterest];
        ...
        tempStr2 = [NSString stringWithFormat:@"%.1f",_Trade_InterestRateDiscount];
        ...
        _textField_6 = [PublicCell updataPublicTableViewCell:Publicstring str2:tempStr2];
        _textField_6.delegate = self;
        _textField_6.tag = tempTag;
        _textField_6.inputAccessoryView=[self addCustomKeyboard];
    }
    else if(indexPathRow == 6){
        tempStr2 = _strDate;
        _textField_7 = [PublicCell updataPublicTableViewCell:Publicstring str2:tempStr2];
        _textField_7.delegate = self;
        _textField_7.tag = tempTag;
        _textField_7.inputAccessoryView=[self addCustomKeyboard];
    }
    return cell;
}
#pragma mark -
...
-(void) ChangeNext:(id) sender{
    if (_NowIsTextFieldLocation != 1000 && _NowIsTextFieldLocation <= 1006) {
...
-(void) ChangePrevious:(id) sender{
    if (_NowIsTextFieldLocation != 1006 && _NowIsTextFieldLocation >= 1000) {
        _NowIsTextFieldLocation++;
        UITextField *textField = [self.view viewWithTag:_NowIsTextFieldLocation];
        [textField becomeFirstResponder];
    }
...
-(void)changeValueIsNull:(id)sender{
    ...
    _textField_3.text = @"";
    _textField_6.text = @"";
    _textField_2.text = @"3.25";
    _textField_4.text = @"4.90";
    _textField_5.text = @"1.0";
    _CPF_LoanMoney = 0;
    _CPF_LoanInterest = 3.25;
    _Trade_LoanMoney = 0;
    _Trade_LoanInterest = 4.90;
    _Trade_InterestRateDiscount = 1.0;
    _LoanDeadline = 0;
    //获取当前时间，日期
    NSDate *currentDate = [NSDate date];
    ...
    [_Combination_TableView reloadData];
}
#pragma mark 计算结果事件
-(void)changeValueIsCalcute:(id)sender{
    [self.view endEditing:YES];
    [self chanceTableViewTopLayoutRangeNormal];
    if (_CPF_LoanMoney>0&&_Trade_LoanMoney>0&&_LoanDeadline>0&&_CPF_LoanInterest>0&&_Trade_LoanInterest>0&&_Trade_InterestRateDiscount>0&&_strDate!=nil) {
        _Section_2_Number = 3;
        _Section_3_Number = _LoanDeadline* 12 + 2;
        _CPF_surplusMoney = _CPF_LoanMoney * 10000;
        _Trade_surplusMoney = _Trade_LoanMoney * 10000;
        
        _CPF_EveryMonthDouble = (_CPF_LoanInterest / 100 / 12);
        _Trade_EveryMonthDouble = (_Trade_LoanInterest / 100 / 12) * _Trade_InterestRateDiscount;
...
-(void)averageCapitalPlusInterest{
    ...
    double CPF_tempLoan = pow( 1 + _CPF_EveryMonthDouble, tempData);
    double Trade_tempLoan = pow( 1 + _Trade_EveryMonthDouble, tempData);
    //每月还款已定(公积金)
    _CPF_EveryMonthMoney = (_CPF_LoanMoney* 10000) * _CPF_EveryMonthDouble * CPF_tempLoan / (CPF_tempLoan-1);
    //每月还款已定(商业)
    _Trade_EveryMonthMoney = (_Trade_LoanMoney* 10000) * _Trade_EveryMonthDouble * Trade_tempLoan / (Trade_tempLoan-1);
    
    //还款总额
    double _TotalMoney = _CPF_EveryMonthMoney * tempData + _Trade_EveryMonthMoney * tempData;
    //累计还款总额
    _PayRental = [NSString stringWithFormat:@"%.2f",_TotalMoney];
    //累计支付利息
    double tempPayInterest = _TotalMoney - (_CPF_LoanMoney* 10000) - (_Trade_LoanMoney* 10000);
    _PayInterest = [NSString stringWithFormat:@"%.2f",tempPayInterest];
    for (NSInteger i = 0; i < tempData; i++) {
        
        //公积金每月利息
        _CPF_EveryLoanInterest = _CPF_surplusMoney * _CPF_EveryMonthDouble;
        //公积金各期还的本金
        _CPF_EveryMonthInterest = _CPF_EveryMonthMoney - _CPF_EveryLoanInterest;
        //公积金剩的本金
        _CPF_surplusMoney = _CPF_surplusMoney - _CPF_EveryMonthInterest;
        
        //商业贷款每月利息
        _Trade_EveryLoanInterest = _Trade_surplusMoney * _Trade_EveryMonthDouble;
        //商业贷款各期还的本金
        _Trade_EveryMonthInterest = _Trade_EveryMonthMoney - _Trade_EveryLoanInterest;
        //商业贷款剩的本金
        _Trade_surplusMoney = _Trade_surplusMoney - _Trade_EveryMonthInterest;
        
        //两个贷款各期总共还的
        double tempTotal = _CPF_EveryMonthInterest + _Trade_EveryMonthInterest;
        //两额贷款总共的利息
        double tempInterest = _CPF_EveryLoanInterest + _Trade_EveryLoanInterest;
        
        NSString *strTemp = [NSString stringWithFormat:@"%.2f/%.2f",tempTotal,tempInterest];
...
-(void)averageCapital{
    ...
    //**公积金支付利息**//
    double CPF_TotalInterest = 0;
    //**商业贷款利息**//
    double Trade_TotalInteresr = 0;
    double tempData  = _LoanDeadline * 12;
    //公积金还款本金
    _CPF_EveryMonthMoney = _CPF_LoanMoney * 10000 / tempData;
    //商业贷款还款本金
    _Trade_EveryMonthMoney = _Trade_LoanMoney * 10000 / tempData;
    ...
    for (NSInteger i = 0; i < tempData; i++) {
        ...
        _CPF_EveryLoanInterest = _CPF_surplusMoney * _CPF_EveryMonthDouble;
        //公积金每期还款
        _CPF_EveryMonthInterest = _CPF_EveryLoanInterest + _CPF_EveryMonthMoney;
        //公积金剩余本金
        _CPF_surplusMoney = _CPF_surplusMoney - _CPF_EveryMonthMoney;
        //公积金还的总利息
        CPF_TotalInterest = CPF_TotalInterest + _CPF_EveryLoanInterest;
        
        //行业贷款每月利息
        _Trade_EveryLoanInterest = _Trade_surplusMoney * _Trade_EveryMonthDouble;
        //商业贷款每期还款
        _Trade_EveryMonthInterest = _Trade_EveryLoanInterest + _Trade_EveryMonthMoney;
        //商业贷款剩余本金
        _Trade_surplusMoney = _Trade_surplusMoney - _Trade_EveryMonthMoney;
        //商业贷款还的总利息
        Trade_TotalInteresr = Trade_TotalInteresr + _Trade_EveryLoanInterest;
        
        //两种贷款的每期还款总和
        double temp_EveryMonthInterest = _CPF_EveryMonthInterest + _Trade_EveryMonthInterest;
        //两种贷款的每月利息总和
        double temp_EveryLoanInterest = _CPF_EveryLoanInterest + _Trade_EveryLoanInterest;
        
        NSString *strTemp = [NSString stringWithFormat:@"%.2f/%.2f",temp_EveryMonthInterest,temp_EveryLoanInterest];
        [_Section_3DataSource addObject:strTemp];
    }
    //累计支付利息
    _PayInterest = [NSString stringWithFormat:@"%.2f",(CPF_TotalInterest + Trade_TotalInteresr)];
    //累计还款总额
    _PayRental = [NSString stringWithFormat:@"%.2f",(CPF_TotalInterest + Trade_TotalInteresr + (_CPF_LoanMoney + _Trade_LoanMoney ) * 10000)];
    //将事件放入到一个数组里
    [self timeDisposalAction];
}
#pragma mark -
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    else if(textField == _textField_6){
        _NowIsTextFieldLocation = 1005;
    }
    else if(textField == _textField_7){
        _NowIsTextFieldLocation = 1006;
    }
...
{
    if (textField == _textField_1) {
        _CPF_LoanMoney = textField.text.doubleValue;
    }
    else if (textField == _textField_2){
        _CPF_LoanInterest = textField.text.doubleValue;
    }
    else if(textField == _textField_3){
        _Trade_LoanMoney = textField.text.doubleValue;
    }
    else if(textField == _textField_4){
        _Trade_LoanInterest = textField.text.doubleValue;
    }
    else if(textField == _textField_5){
        _Trade_InterestRateDiscount = textField.text.doubleValue;
    }
    else if(textField == _textField_6){
        _LoanDeadline = textField.text.doubleValue;
    }
}
#pragma mark 当某个TextField将要显示并实现的时候调用，主要为了判断当是日期选择器那个TextField时，显示日期选择器
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if (_textField_7 == textField) {
        _textField_7.inputView = _datePicker;
    }
    return  YES;
}
#pragma mark -
...
-(void)chanceTableViewTopLayoutRangeBeginNotFirst{
    if (Commonality_OpenQuipment_iPhone_4s) {
        ...
        if (tempTextFieldLocation == 6) {
            ...
            [self SetTheAnimation:Commonality_TableView_SectionRowHigh * (tempTextFieldLocation - 1)];
        }
       else{
           [self chanceTableViewTopLayoutRangeNormal];
       }
        ...
        NSInteger tempTextFieldLocation = _NowIsTextFieldLocation - 1000;
        if (tempTextFieldLocation == 6){
            [self SetTheAnimation:Commonality_TableView_SectionRowHigh * (tempTextFieldLocation - 2 )];
        }
        else if (tempTextFieldLocation > 3) {
            [self SetTheAnimation:Commonality_TableView_SectionRowHigh * (tempTextFieldLocation - 2)];
        }
        else if (tempTextFieldLocation == 1){
            [self chanceTableViewTopLayoutRangeNormal];
        }
    }
    else if (Commonality_OpenQuipment_iPhone_6Plus_6sPlus){
        NSInteger tempTextFieldLocation = _NowIsTextFieldLocation - 1000;
        if (tempTextFieldLocation > 3) {
            [self SetTheAnimation:Commonality_TableView_SectionRowHigh * (tempTextFieldLocation - 3)];
        }else if (tempTextFieldLocation == 1){
            [self chanceTableViewTopLayoutRangeNormal];
        }
    }
...
-(void)SetTheAnimation:(NSInteger)chanceLayoutValue{
    ...
    //设置视图的透明度
    _Combination_TableView_TopLayoutRange.constant = - chanceLayoutValue;
