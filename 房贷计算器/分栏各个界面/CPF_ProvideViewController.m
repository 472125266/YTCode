...
#import "CPF_ProvideViewController.h"


@interface CPF_ProvideViewController ()

@end

@implementation CPF_ProvideViewController
#pragma mark -
- (void)viewDidLoad {
    [super viewDidLoad];
    
    //数据初始化
    [self CPF_InitViewNeedThing];
}
#pragma mark -
#pragma marrk 初始化界面需要的一些数据，设置TableView的代理，以及界面上的一些控件
-(void)CPF_InitViewNeedThing{
    self.navigationItem.title = @"公积金贷款";
    _Section_1DataSource = [[NSMutableArray alloc]initWithObjects:CPF_Section_1_DataSourceArray, nil];
    _Section_2DataSource = [[NSMutableArray alloc]initWithObjects:CPF_Section_2_DataSourceArray, nil];
    ...
    //TableView的代理设置
    _CPF_tableView.delegate = self;
    _CPF_tableView.dataSource = self;
    //公积金默认利率为3.25%
    _LoanInterest = CPF_AccumulationFund_Default;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *strCell = @"CPF_Cell";
...
-(void) ChangeNext:(id) sender{
    if (_NowIsTextFieldLocation != 1000 && _NowIsTextFieldLocation <= 1003) {
...
-(void) ChangePrevious:(id) sender{
    if (_NowIsTextFieldLocation != 1003 && _NowIsTextFieldLocation >= 1000) {
        _NowIsTextFieldLocation++;
            UITextField *textField = [self.view viewWithTag:_NowIsTextFieldLocation];
            [textField becomeFirstResponder];
    }
...
-(void)changeValueIsNull:(id)sender{
    ...
    [_CPF_tableView reloadData];
}
#pragma mark 计算结果事件
-(void)changeValueIsCalcute:(id)sender{
    [self.view endEditing:YES];
    [self chanceTableViewTopLayoutRangeNormal];
    if (_LoanDeadline>0&&_LoanInterest>0&&_LoanMoney>0&&_strDate!=nil) {
        _Section_2_Number = 2;
        _Section_3_Number = _LoanDeadline* 12 + 2;
        _surplusMoney = _LoanMoney * 10000;
        _EveryMonthDouble = ( _LoanInterest / 100 / 12 );
...
-(void)SetTheAnimation:(NSInteger)chanceLayoutValue{
    ...
    //设置视图的透明度
    _CPF_TableView_TopLayoutRange.constant = - chanceLayoutValue;
